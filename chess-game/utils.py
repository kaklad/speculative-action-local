...
def resolve_local_model_name(config: dict[str, Any], model_name: str) -> str:
    """Resolve a configured local model id to a path under models.local.root."""
    if model_name.startswith(("/", "./", "../")):
        return model_name

    local_models = config.get("models", {}).get("local", {})
    models_root = local_models.get("root", "./models")
    return str(Path(models_root) / model_name)
...
def get_llm_client_and_model(
    ...
    """Build an OpenAI-compatible client and resolve the model name for a provider."""
    from openai import OpenAI

    provider = normalize_provider(provider)
    if provider == "local":
        local_api = config.get("api", {}).get("local", {})
        base_url = local_api.get(f"{role}_base_url") or local_api.get("base_url")
        if not base_url:
            raise ValueError(f"Missing api.local.{role}_base_url or api.local.base_url in config.yml")
        return (
            OpenAI(base_url=base_url, api_key=local_api.get("key", "EMPTY")),
            resolve_local_model_name(config, model_name),
        )

    if provider == "openai":
        return OpenAI(api_key=config["api"]["openai"]["key"]), model_name

    raise ValueError(f"Unknown provider: {provider}")


def chat_completion_options(provider: str, include_reasoning_effort: bool = True) -> dict[str, Any]:
    """Return provider-specific chat.completions options."""
    provider = normalize_provider(provider)
    options: dict[str, Any] = {"n": 1}
    if include_reasoning_effort and provider == "openai":
